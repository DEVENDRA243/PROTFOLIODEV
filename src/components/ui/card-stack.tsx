...
import { ExternalLink as SquareArrowOutUpRight } from "lucide-react";
...
export function CardStack<T extends CardStackItem>({
  ...
  // listen for custom events to control the stack externally
  React.useEffect(() => {
    const handleSetIndex = (e: any) => {
      if (typeof e.detail === 'number') {
        setActive(wrapIndex(e.detail, len));
      }
    };
    window.addEventListener('set-card-stack-index', handleSetIndex);
    return () => window.removeEventListener('set-card-stack-index', handleSetIndex);
  }, [len]);
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          {activeItem.href ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-2.5 px-5 py-2 bg-[#1a1a1a] hover:bg-[#252525] border border-white/5 rounded-full transition-all duration-300 group shadow-2xl active:scale-95"
              aria-label="Open link"
            >
              <span className="text-[11px] font-black text-white tracking-[0.15em] uppercase transition-all group-hover:text-accent [text-shadow:_0.5px_0_0_#ff0000,_ -0.5px_0_0_#0000ff]">
                Go Live
              </span>
              <SquareArrowOutUpRight className="h-3.5 w-3.5 text-white/50 group-hover:text-accent group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300" />
            </a>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <div className="relative h-full w-full bg-[#050505]">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-10 pb-12">
        <div className="truncate text-2xl font-bold text-white tracking-tight mb-2">
          {item.title}
        </div>
        {item.description ? (
          <div className="line-clamp-2 text-sm text-white/80 font-medium max-w-[90%] leading-snug">
            {item.description}
          </div>
        ) : null}
      </div>
